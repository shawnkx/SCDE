...
class PreTrainedTokenizer(object):
    ...
    def truncate_sequences(self, ids, pair_ids=None, num_tokens_to_remove=0, truncation_strategy='longest_first', stride=0):
        ...
        if truncation_strategy == 'longest_first':
            ...
            #print(len(ids), num_tokens_to_remove, len(pair_ids))
            assert len(ids) > num_tokens_to_remove
            window_len = min(len(ids), stride + num_tokens_to_remove)
            overflowing_tokens = ids[-window_len:]
            ids = ids[:-num_tokens_to_remove]
        elif truncation_strategy == 'first_left':
            #print(len(ids), num_tokens_to_remove, len(pair_ids))
            assert len(ids) > num_tokens_to_remove
            window_len = min(len(ids), stride + num_tokens_to_remove)
            overflowing_tokens = ids[:window_len]
        ...
        elif truncation_strategy == 'longest':
            ...
            raise ValueError("Truncation_strategy should be selected in ['longest_first', 'only_first', 'only_second', 'do_not_truncate', 'longest']")
